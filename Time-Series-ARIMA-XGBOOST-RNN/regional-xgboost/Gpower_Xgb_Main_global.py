...
N_rows = 2492  # Total number of rows in mp-global-avg.txt (including all data including 2025)
parse_dates = [['Date', 'Time']]
filename = "../mp-avg/mp-global-avg.txt"
...
storage = optuna.storages.RDBStorage(url="sqlite:///global_optuna_study.db")
...
if len(study.trials) > 0:
    print("✅ Found existing Optuna study for Global with completed trials.")
    ...
    print("Starting new Optuna optimization for Global XGBoost parameters...")
    ...
    print(f"\nOptuna Optimization Results for Global:")
...
print('\n-----XGBoost Using All Numeric Features with Optuna-Optimized Parameters for Global-----')
print('---Feature importance with optimized model---')
fig_allFeatures = xgb_importance(
    df, val_ratio, optimized_xgb_params, ntree, early_stop, 'Global_Optuna_Optimized_All_Features')
...
print("Global data date range after preprocessing:")
...
print('\n-----XGBoost on datetime information with Optuna-Optimized Parameters for Global-----\n')
...
print("Training final Global model with Optuna-optimized parameters...")
...
print('-----XGBoost with Optuna-Optimized Parameters for Global Microplastic Concentration------')
print('---Testing with optimized model---')

# Create simple plot without forecast data
Y_combined = pd.concat([Y, Y_test])
ax = Y_combined[plot_start:].plot(label='observed', figsize=(15, 10), color='#FFC0CB', alpha=0.8)
Y_hat.plot(label="test_predicted", ax=ax, color='#FF0000', linewidth=2)

# Highlight test period
ax.fill_betweenx(ax.get_ylim(), pd.to_datetime(Y_test.index[0]), Y_test.index[-1],
                 alpha=0.1, color='#FF0000', zorder=-1, label='Test Period')

ax.set_xlabel('Time')
ax.set_ylabel('Microplastic Concentration')
ax.set_title('Global Optuna_Optimized_Model - Microplastic Concentration Prediction')
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('Global_Optuna_Optimized_Model.png', dpi=300)
plt.close()
print("Global test prediction plot saved as: Global_Optuna_Optimized_Model.png")
...
optimized_params_df.to_csv('global_optuna_optimized_parameters.csv', index=False)
print("Global optimized parameters saved to 'global_optuna_optimized_parameters.csv'") 