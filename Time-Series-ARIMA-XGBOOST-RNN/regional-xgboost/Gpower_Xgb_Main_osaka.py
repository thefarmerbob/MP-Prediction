...
from util import *
from myXgb import xgb_importance, xgb_data_split, xgb_forecasts_plot_with_actual
...
N_rows = 2491  # Total number of rows in mp-osaka-avg.txt
filename = "../regional-averages/mp-osaka-avg.txt"
...
storage = optuna.storages.RDBStorage(url="sqlite:///osaka_optuna_study.db")
study = optuna.create_study(
    direction='minimize', 
    study_name='osaka_xgb_optimization',
    storage=storage,
    load_if_exists=True
)
...
if len(study.trials) > 0:
    print("✅ Found existing Optuna study for Osaka with completed trials.")
    ...
    print("Starting new Optuna optimization for Osaka XGBoost parameters...")
    ...
    print(f"\nOptuna Optimization Results for Osaka:")
...
print('\n-----XGBoost Using All Numeric Features with Optuna-Optimized Parameters for Osaka-----')
print('---Feature importance with optimized model---')
fig_allFeatures = xgb_importance(
    df, val_ratio, optimized_xgb_params, ntree, early_stop, 'Osaka_Optuna_Optimized_All_Features')
...
print("Osaka data date range after preprocessing:")
...
print('\n-----XGBoost on datetime information with Optuna-Optimized Parameters for Osaka-----\n')
...
print("Training final Osaka model with Optuna-optimized parameters...")
...
print('-----XGBoost with Optuna-Optimized Parameters for Osaka Microplastic Concentration------')
print('---Testing with optimized model---')

# Create simple plot without forecast data
Y_combined = pd.concat([Y, Y_test])
ax = Y_combined[plot_start:].plot(label='observed', figsize=(15, 10), color='#FFA500', alpha=0.5)
Y_hat.plot(label="test_predicted", ax=ax, color='#FF7F50', linewidth=2)

# Highlight test period
ax.fill_betweenx(ax.get_ylim(), pd.to_datetime(Y_test.index[0]), Y_test.index[-1],
                 alpha=0.1, color='#FF7F50', zorder=-1, label='Test Period')

ax.set_xlabel('Time')
ax.set_ylabel('Microplastic Concentration')
ax.set_title('Osaka Optuna_Optimized_Model - Microplastic Concentration Prediction')
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('Osaka_Optuna_Optimized_Model.png', dpi=300)
plt.close()
print("Osaka test prediction plot saved as: Osaka_Optuna_Optimized_Model.png")

# Calculate and display final model performance metrics
test_rmse = np.sqrt(mean_squared_error(Y_test, Y_hat))
print(f"\nFinal Osaka Model Performance:")
...
optimized_params_df.to_csv('osaka_optuna_optimized_parameters.csv', index=False)
print("Osaka optimized parameters saved to 'osaka_optuna_optimized_parameters.csv'") 