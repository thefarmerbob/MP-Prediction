...
N_rows = 2491  # Total number of rows in mp-tokyo-avg.txt
...
storage = optuna.storages.RDBStorage(url="sqlite:///tokyo_optuna_study.db")
study = optuna.create_study(
    direction='minimize', 
    study_name='tokyo_xgb_optimization',
    storage=storage,
    load_if_exists=True
)
...
if len(study.trials) > 0:
    print("✅ Found existing Optuna study for Tokyo with completed trials.")
    ...
    print("Starting new Optuna optimization for Tokyo XGBoost parameters...")
    ...
    print(f"\nOptuna Optimization Results for Tokyo:")
...
print('\n-----XGBoost Using All Numeric Features with Optuna-Optimized Parameters for Tokyo-----')
print('---Feature importance with optimized model---')
fig_allFeatures = xgb_importance(
    df, val_ratio, optimized_xgb_params, ntree, early_stop, 'Tokyo_Optuna_Optimized_All_Features')
...
print("Tokyo data date range after preprocessing:")
...
print('\n-----XGBoost on datetime information with Optuna-Optimized Parameters for Tokyo-----\n')
...
print("Training final Tokyo model with Optuna-optimized parameters...")
...
print('-----XGBoost with Optuna-Optimized Parameters for Tokyo Microplastic Concentration------')
print('---Testing with optimized model---')

# Create simple plot without forecast data
Y_combined = pd.concat([Y, Y_test])
ax = Y_combined[plot_start:].plot(label='observed', figsize=(15, 10), color='purple', alpha=0.5)
Y_hat.plot(label="test_predicted", ax=ax, color='purple', linewidth=2)

# Highlight test period
ax.fill_betweenx(ax.get_ylim(), pd.to_datetime(Y_test.index[0]), Y_test.index[-1],
                 alpha=0.1, color='purple', zorder=-1, label='Test Period')

ax.set_xlabel('Time')
ax.set_ylabel('Microplastic Concentration')
ax.set_title('Tokyo Optuna_Optimized_Model - Microplastic Concentration Prediction')
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('Tokyo_Optuna_Optimized_Model.png', dpi=300)
plt.close()
print("Tokyo test prediction plot saved as: Tokyo_Optuna_Optimized_Model.png")

# Calculate and display final model performance metrics
test_rmse = np.sqrt(mean_squared_error(Y_test, Y_hat))
print(f"\nFinal Tokyo Model Performance:")
...
optimized_params_df.to_csv('tokyo_optuna_optimized_parameters.csv', index=False)
print("Tokyo optimized parameters saved to 'tokyo_optuna_optimized_parameters.csv'") 