...
print("Adding reduced temporal features...")
...
if len(study.trials) > 0:
    print("✅ Found existing Optuna study with completed trials.")
    ...
    print("Starting new Optuna optimization for key XGBoost parameters...")
...
# XGBoost parameters with Optuna-optimized values (from best trial)
# Best RMSE: 2.111101 with parameters: learning_rate=0.0377, max_depth=6, n_estimators=900
optimized_xgb_params = {
    ...
    'n_estimators': 3000,  # Optuna-optimized
    'reg_alpha': 27.12,  # Fixed
    'reg_lambda': 10.34,  # Fixed
    'gamma': 2.24,  # Fixed
    'tree_method': 'auto',  # Fixed
...
print('\n-----XGBoost Using All Numeric Features with Optuna-Optimized Parameters-----')
print('---Feature importance with optimized model---')
fig_allFeatures = xgb_importance(
    df, val_ratio, optimized_xgb_params, ntree, early_stop, 'Optuna_Optimized_All_Features')
...
print("Data date range after preprocessing:")
...
# get splited data - only train and test, no forecast
_, df_test, df = xgb_data_split(
    df, bucket_size, df.index[-1].strftime('%Y-%m-%d %H:%M:%S'), 0, test_start_date, encode_cols)
print('\n-----XGBoost on datetime information with Optuna-Optimized Parameters-----\n')
...
print('-----XGBoost with Optuna-Optimized Parameters for Microplastic Concentration------')
print('---Testing with optimized model---')

# Create simple plot without forecast data
Y_combined = pd.concat([Y, Y_test])
ax = Y_combined[plot_start:].plot(label='observed', figsize=(15, 10), color='blue', alpha=0.7)
...
ax.fill_betweenx(ax.get_ylim(), pd.to_datetime(Y_test.index[0]), Y_test.index[-1],
                 alpha=0.1, color='orange', zorder=-1, label='Test Period')

ax.set_xlabel('Time')
ax.set_ylabel('Microplastic Concentration')
ax.set_title('Optuna_Optimized_Model - Microplastic Concentration Prediction')
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('Optuna_Optimized_Model.png', dpi=300)
plt.close()
print("Test prediction plot saved as: Optuna_Optimized_Model.png")
...
optimized_params_df.to_csv('optuna_optimized_parameters.csv', index=False)
print("Optimized parameters saved to 'optuna_optimized_parameters.csv'")
